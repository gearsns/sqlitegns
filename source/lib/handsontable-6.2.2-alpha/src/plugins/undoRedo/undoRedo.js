...
import { stopImmediatePropagation, stopPropagation } from './../../helpers/dom/event'; // 2023/05/16
...
function onBeforeKeyDown(event) {
  ...
  // 2023/05/16 編集中の場合はundo,redoしない
  const activeEditor = instance.getActiveEditor()
  if (activeEditor && (activeEditor.isWaiting() || activeEditor.isOpened())){
    return;
  }
  if (ctrlDown) {
    if (event.keyCode === 89 || (event.shiftKey && event.keyCode === 90)) { // CTRL + Y or CTRL + SHIFT + Z
      ...
      stopPropagation(event) // 2023/05/16 Event停止
